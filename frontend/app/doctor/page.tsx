    ...
    <div className="min-h-screen bg-gray-50">
      <nav className="bg-white border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          ...
          <button onClick={() => { logout(); router.push('/login'); }} className="text-sm text-red-500 hover:text-red-700">
            Logout
          </button>
        </div>
      </nav>
      ...
      <div className="max-w-6xl mx-auto mt-8 px-4 grid grid-cols-3 gap-6">
        ...
        <div className="col-span-1">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={fetchPatients} className="text-xs text-blue-600 hover:text-blue-800">Refresh</button>
          </div>

          {patients.some(p => p.urgency === 'High' && p.status === 'waiting') && (
            <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 mb-3">
              <p className="text-red-700 text-xs font-medium">🚨 High urgency patient(s) in queue — please attend immediately</p>
            </div>
          )}
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <div className="flex gap-1">
                      {patient.urgency === 'High' && <span className="text-xs px-2 py-1 rounded-full font-medium bg-red-50 text-red-700">🚨 High</span>}
                      {patient.urgency === 'Medium' && <span className="text-xs px-2 py-1 rounded-full font-medium bg-yellow-50 text-yellow-700">⚠️ Medium</span>}
                      {patient.urgency === 'Low' && <span className="text-xs px-2 py-1 rounded-full font-medium bg-green-50 text-green-700">✓ Low</span>}
        ...
        <div className="col-span-2">
          {!selectedPatient ? (
            ...
            <div className="bg-white rounded-2xl border border-gray-100 p-6 overflow-y-auto max-h-screen">
              ...
              <div className="bg-purple-50 border border-purple-100 rounded-xl p-3 mb-3">
                <p className="text-xs font-medium text-purple-600 mb-1">🤖 AI Pre-Consultation Summary</p>
                {summaryLoading ? (
              ...
              {success && (
                <div className="bg-green-50 text-green-700 px-4 py-2 rounded-lg mb-3 text-sm font-medium">
                  {success}
                </div>
              )}

              <div className="bg-gray-50 rounded-xl p-3 mb-3">
                <p className="text-xs font-medium text-gray-500 mb-2">Voice to Prescription</p>
                <div className="flex items-center gap-3">
                  ...
                  {recording && <span className="text-red-500 text-xs font-medium animate-pulse">Recording...</span>}
                  {audioBlob && !recording && <span className="text-green-600 text-xs font-medium">✓ Audio captured</span>}
                </div>
                {audioBlob && !recording && (
                  <div className="mt-2">
